...
public class MyConfigListWidget extends ContainerObjectSelectionList<MyConfigListWidget.Entry> {
    ...
    public void addButton(Option<?> option, String indent) {
        this.addEntry(new ButtonEntry(Button.builder(Component.translatable("dynamichud.config.value." + option.getValue().toString().toLowerCase()), btn -> {option.setToNextValue();}).bounds(0, 0, 100, 20).build(), option, indent));
    }
